...
$("#OSL_Modal").on("shown.bs.modal", function () {
  if (
    fileName === "" &&
    $("select[id=metersPerPixel] option:selected").val() === ""
  ) {
...
$("#OSL_Modal").on("hidden.bs.modal", function () {
  ...
  document.getElementById("fupload").nextElementSibling.innerText =
    "Choose file";
  $("span[id=error_oslUploadbtn]").text("");
  $("button[id=oslUploadbtn]").prop("disabled", true);
...
function Add_OSL() {
  ...
  $("div[id=oslindexForm]").css("display", "none");
              ...
              function (evt) {
                if (evt.lengthComputable) {
                  ...
                  if (percentComplete === 100) {
                    $("span[id=error_oslUploadbtn]").text(
                      "File Transfer Complete -->> Processing File "
                    );
                  }
                }
              },
...
function Edit_OSL(data) {
  ...
  $("div[id=oslindexForm]").css("display", "block");
  $("select[id=oslindex]").val(data.index);
  $("button[id=oslSubmitbtn]").on("click", function () {
    ...
    jsonObject["coordinateSystemId"] = $(
      "input[id=CoordinateSystemId_txt]"
    ).val();
    ...
    jsonObject["index"] = $("select[id=oslindex] option:selected").val();
    if (!$.isEmptyObject(jsonObject)) {
      $.ajax({
        url:
          SiteURLconstructor(window.location) + "/api/BackgroundImage/Update",
        ...
        error: function (error) {
          $("span[id=error_oslUploadbtn]").text(error);
          $("button[id=oslSubmitbtn]").prop("disabled", false);
          //console.log(error);
        },
  ...
  $.each(arrayColums[0], function (key, value) {
    ...
    if (/name/i.test(key)) {
      ...
      tempc = {
        ...
        mRender: function (data, type, full) {
          return (
            '<button type="button" class="btn btn-light btn-sm mx-1 pi-iconEdit ml-p5rem" data-toggle="modal" name="oslEdit" title="Edit OSL"></button>' +
    ...
    $.ajax({
      ...
      error: function (error) {
        $("span[id=error_apisubmitBtn]").text(
          data.name + " " + data.messageType + " Connection was not Updated"
        );
        //console.log(error);
      },
      faulure: function (fail) {
