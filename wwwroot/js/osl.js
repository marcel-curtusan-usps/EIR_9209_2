...
function Edit_OSL(data) {
  ...
  $('button[id=oslSubmitbtn]').on('click', function() {
    ...
    if (!$.isEmptyObject(jsonObject)) {
      $.ajax({
        url: SiteURLconstructor(window.location) + '/api/BackgroundImage/Update',
  ...
  $.each(arrayColums[0], function(key, value) {
    ...
    if (/name/i.test(key)) {
      ...
      tempc = {
        ...
        width: '15%',
        mRender: function(data, type, full) {
          return '<button type="button" class="btn btn-light btn-sm mx-1 pi-iconEdit ml-p5rem" data-toggle="modal" name="oslEdit" title="Edit OSL"></button>' + '<button type="button" class="btn btn-light btn-sm mx-1 pi-trashFill ml-p5rem" data-toggle="modal" name="oslDelete" title="Delete OSL"</button>';
        }
      };
    } else {
      tempc = {
