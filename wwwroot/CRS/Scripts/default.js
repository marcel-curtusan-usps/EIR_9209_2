...
let errorTimeout;
let currentUser = {};
const errorTimeLimit = 15000; // 15 seconds
let inactivityTimeout;
const inactivityTimeLimit = 15000; // should be 15 seconds
...
async function tacsTime() {
  ...
  // Calculate the postal time using the given conversion equation
  const postalTime = hours + Math.floor((minutes * 60 + seconds) / 36) / 100;

  // Format the postal time to two decimal places
  const formattedPostalTime = postalTime.toFixed(2);

  // Update the TACS time display
  $('span[id=tacsTime]').text(formattedPostalTime);
}
// Update the date and time every second
setInterval(updateDateTime, 1000);
setInterval(tacsTime, 1000);
...
async function loadTopCodes(codesList) {
  try {
    ...
    limitedCodes.forEach(code => {
      const codeElement = $('<div>').addClass('col-2').append($('<button>').addClass('topCodeButton btn btn-light').text(code).attr('data-tranCode', '011'));
