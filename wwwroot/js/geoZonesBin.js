...
async function addBinFeature(data) {
  try {
    await findBinZoneLeafletIds(data.properties.id).then(leafletIds => {}).catch(error => {
...
async function updateBinFeature(data) {
  try {
    await findBinZoneLeafletIds(data.zoneId).then(leafletIds => {
...
let czzonetop_row_template = '<tr data-id="{zoneId}"><td style="width: 22%;">Bin Zone Name</td><td>{zoneName}</td></tr>' + '<tr><td>Bins Configured</td><td>{AssignedBins}</td></tr>' + '<tr><td>Full Bins</td><td>{fullbins}</td></tr>';
