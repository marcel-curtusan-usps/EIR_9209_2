      ...
      $.each(arrayColums[0], function(key) {
        ...
        if (/Action/i.test(key)) {
          tempc = {
            ...
            mDataProp: key,
            mRender: function(data, full) {
              return '<button type="button" class="btn btn-light btn-sm mx-1 pi-iconEdit" data-toggle="modal" name="editdacodetocrafttype" data-id="' + full.id + '"title="Edit Connection Type"></button>' + ' <button type="button" class="btn btn-light btn-sm mx-1 pi-trashFill" data-toggle="modal" name="deletedacodetocrafttype" data-id="' + full.id + '"title="Delete Connection Type"></button>';
            }
          };
        }
        columns.push(tempc);
