      ...
      if (/Action/i.test(key)) {
        tempc = {
          ...
          mRender: function(data, full) {
            return '<button type="button" class="btn btn-light btn-sm mx-1 pi-iconEdit" data-toggle="modal" name="editconnectiontype" data-id="' + full.id + '"title="Edit Connection Type"></button>' + ' <button type="button" class="btn btn-light btn-sm mx-1 pi-trashFill" data-toggle="modal" name="deleteconnectiontype" data-id="' + full.id + '"title="Delete Connection Type"></button>' + ' <button type="button" class="btn btn-light btn-sm mx-1 bi-plus" data-toggle="modal" name="addconnectionsubtype" data-id="' + full.id + '"title="Add New Connection Subtype"></button>';
          }
        };
      }
      columns.push(tempc);
    ...
    $.each(arrayColums[0], function(key, value) {
      ...
      if (/Action/i.test(key)) {
        tempc = {
          ...
          mDataProp: key,
          mRender: function() {
            return '<button type="button" class="btn btn-light btn-sm mx-1 pi-iconEdit" data-toggle="modal" name="editconnectionsubtype" data-id="' + parentid + '"title="Edit Connection Subtype"></button>' + '<button type="button" class="btn btn-light btn-sm mx-1 pi-trashFill" data-toggle="modal" name="deleteconnectionsubtype" data-id="' + parentid + '"title="Delete Connection Subtype"></button>';
          }
        };
      }
      columns.push(tempc);
...
function enableConnectionSubTypeSubmit() {
  if ($('input[id=modalConnTypeID]').hasClass('is-valid') && $('input[id=descValueID]').hasClass('is-valid') && $('input[id=baseURLValueID]').hasClass('is-valid')) {
    $('button[id=conntypevalue]').prop('disabled', false);
  } else {
    $('button[id=conntypevalue]').prop('disabled', true);
  }
